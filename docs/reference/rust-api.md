...
| Method | Description |
| --- | --- |
| `client.queue(name)` | Create a queue handle |
| `queue.create_queue().await` | Create the queue |
| `queue.purge_queue().await` | Remove active queue contents |
| `queue.delete_queue().await` | Delete the queue |
